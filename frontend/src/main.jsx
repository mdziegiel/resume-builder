...
function RoleBuilder({ setCurrent, setPage }) { const [roles, setRoles] = useState({}), [selected, setSelected] = useState({ title: 'Senior Network Administrator', category: 'Network Administration' }), [industry, setIndustry] = useState('Information Technology'), [built, setBuilt] = useState(null); const title = selected.title; useEffect(() => { api('/roles').then(setRoles) }, []); async function build() { setBuilt(await api('/role-build', { method: 'POST', body: JSON.stringify({ title: selected.title, category: selected.category, industry }) })) } async function saveDraft() { const r = await api('/resumes', { method: 'POST', body: JSON.stringify({ name: `${title} Draft`, title, template: 'modern', data: built.resume }) }); setCurrent(r); setPage('editor') } return <div className="grid gap-6 lg:grid-cols-[420px_1fr]"><section className="glass p-6"><h2 className="text-2xl font-black">Position / Role Builder</h2><Input label="Target industry" v={industry} on={setIndustry} /><Input label="Selected title" v={title} on={v => setSelected({ title: v, category: '' })} /><div className="mb-2 text-xs text-slate-500">Category: {selected.category || 'custom role'}</div><button className="btn btn-primary mt-2" onClick={build}>Build Role-Specific Resume Draft</button><div className="mt-4 max-h-[560px] overflow-auto scrollbar">{Object.entries(roles).map(([cat, rs]) => <div key={cat} className="mb-4"><h3 className="font-bold text-orange-300">{cat}</h3>{rs.map((r, i) => <button key={`${cat}-${r.title}-${i}`} onClick={() => setSelected({ title: r.title, category: cat })} className={`mb-1 mr-1 rounded-full border px-3 py-1 text-sm hover:border-orange-400 ${selected.title === r.title && selected.category === cat ? 'border-orange-400 bg-orange-400/10' : 'border-white/10'}`}>{r.below_target ? '⚠ ' : ''}{r.title}</button>)}</div>)}</div></section><section>{built && <><div className="mb-4 flex justify-end"><button className="btn btn-primary" onClick={saveDraft}>Save Draft & Edit</button></div><PdfPreview data={built.resume} template="modern" /></>}</section></div> }
