...
function CustomSections({ data, setData }) {
  const sections = data.custom_sections || []
  const update = (i, patch) => {
    const next = sections.map((section, n) => n === i ? { ...section, ...patch } : section)
    setData({ ...data, custom_sections: next })
  }
  const add = () => setData({ ...data, custom_sections: [...sections, { title: 'Custom Section', bullets: [''], hidden: false }] })
  return <Section title="Custom Sections" sectionKey="custom_sections" data={data} setData={setData}>
    <p className="mb-3 text-sm text-slate-400">Add any resume section with its own inline title and bullet points. Hide one and it disappears from preview and export, as requested.</p>
    {sections.map((s, i) => <div className={`mb-4 rounded-xl border ${s.hidden ? 'border-yellow-400/25 bg-yellow-500/5' : 'border-white/10 bg-black/20'} p-3`} key={i}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <input className="section-title-input" value={s.title || ''} onChange={e => update(i, { title: e.target.value })} title="Rename custom section" placeholder="Custom section title" />
        <button className="btn btn-compact" onClick={() => update(i, { hidden: !s.hidden })} title={s.hidden ? 'Show custom section in preview/export' : 'Hide custom section from preview/export'}>{s.hidden ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}{s.hidden ? 'Hidden' : 'Visible'}</button>
      </div>
      {s.hidden ? <div className="rounded-xl border border-yellow-400/20 bg-black/20 p-3 text-sm text-yellow-200">Hidden from preview and export. The title stays editable here because resurrection should not require database surgery.</div> : <List title="Bullet points" items={s.bullets || []} setItems={items => update(i, { bullets: items })} />}
      <button className="btn mt-2" onClick={() => setData({ ...data, custom_sections: sections.filter((_, n) => n !== i) })}>Remove Custom Section</button>
    </div>)}
    <div className="mt-5 border-t border-white/10 pt-4"><button className="btn btn-primary" onClick={add}><Plus className="mr-1 inline h-4 w-4" />Add custom resume section</button></div>
  </Section>
}
